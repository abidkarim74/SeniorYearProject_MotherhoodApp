...
const BotLeftBar = ({ onSelectConversation, onNewChat, currentChatId }: BotLeftBarProps) => {
  ...
  const fetchConversations = async () => {
    try {
      ...
      const transformedConversations: Conversation[] = response.map((conv: ApiConversation) => ({
        id: conv.id,
        title: conv.topic || 'Untitled Conversation', // Use topic as title
        lastMessage: 'Click to continue conversation', // Placeholder, you might want to fetch last message separately
        time: 'Recently', // You can format this based on actual conversation timestamp
        unread: 0 // You can add unread count if your API provides it
      }));
