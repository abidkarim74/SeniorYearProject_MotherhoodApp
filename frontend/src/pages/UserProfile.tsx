            ...
            <div className="md:col-span-4 lg:col-span-4">
              <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden sticky top-4">
                <div className={`px-4 py-6 text-white text-center ${isOwnProfile ? "bg-gradient-to-r from-[#e5989b] to-[#d88a8d]" : "bg-gradient-to-r from-blue-500 to-blue-600"}`}>
                  <div className="relative inline-block">
                    <img src={previewUrl || mother.profile_pic || "https://cdn-icons-png.flaticon.com/512/149/149071.png"} alt={`${mother.firstname} ${mother.lastname}`} className="w-28 h-28 rounded-xl object-cover border-4 border-white/20 shadow-lg mx-auto"/>
                    {isOwnProfile && (
                      <label className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-green-400 flex items-center justify-center cursor-pointer border-2 border-white">
                  ...
                  <p className="text-white/80 text-sm">@{mother.username}</p>
                  {isOwnProfile ? (
                    <button onClick={() => setShowUpdateModal(true)} className="mt-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm px-3 py-1 rounded-lg flex items-center gap-2 transition-all duration-300 border border-white/30 mx-auto text-sm">
                      <Edit className="w-3 h-3" />
                      <span className="font-medium">Edit Profile</span>
                    </button>
                  ) : (
